...
export default function ProductListingClient({
  ...
  const buildPaginationUrl = (pageNum: number) => {
    ...
    if (Array.isArray(categoryParam)) {
      categoryParam.forEach(cat => params.append('category', cat));
    ...
    return (
      <div className="flex justify-center mt-8 gap-1">
        ...
        {paginationPages.map(pageNumber => (
          <Link key={pageNumber} href={buildPaginationUrl(pageNumber)} aria-current={currentPage === pageNumber ? "page" : undefined}>
            <Button variant={currentPage === pageNumber ? "default" : "outline"} size="sm">
              {pageNumber}
            </Button>
          </Link>
        ))}
